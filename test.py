...
os.system('python ~/PycharmProjects/my-neural-style/faster_neural_style.py --style_folder=%s --styles %s %s --learning_rate=%f --iterations=%d --batch_size=%d %s %s %s --style_weight=%d --content_weight=%d --checkpoint_iterations=%d --width=%d --height=%d --checkpoint_output=%s --test_img=%s --output=%s --model_save_dir=%s --print_iterations=%d %s %s %s --style_semantic_mask_dirs %s --semantic_masks_num_layers=%d --mask_folder=%s'
          % (style_folder, ' '.join(styles), texture_synthesis_only_string, learning_rate, iterations, batch_size, use_mrf_string, use_johnson_string, multi_style_offset_only_string, style_weight, content_weight, checkpoint_iterations, width, height, checkpoint_output, test_img, output, model_save_dir, print_iteration, do_restore_and_train_string, do_restore_and_generate_string, use_semantic_masks_string, ' '.join(style_semantic_mask_dirs), semantic_masks_num_layers, mask_folder))
