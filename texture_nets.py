...
def generator_net(input_noise_z):
    ...
    with tf.get_default_graph().name_scope('texture_nets'):
        noise_joined = input_noise_z[-1]
        current_channels = 8
        channel_step_size = 8
        for noise_layer in input_noise_z[1:]: # TODO: THERE MAY BE A BUG HERE. DOUBLE CHECK TEH ORDER
...
def conv_relu_layers(name, input_layer, kernel_size, out_channels, relu_leak = 0.01):
    ...
    with tf.get_default_graph().name_scope(name):
        ...
        # Apply normalization to conv here in the future.
        batch_norm = spatial_batch_norm(conv)
        relu = neural_util.leaky_relu(batch_norm, .01)
        return relu
        # relu = neural_util.leaky_relu(conv, relu_leak)
        # return relu
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for i in range(iterations):
                ...
                feed_dict = {}
                noise = noise_pyramid_w_content_img(m, batch_size, content_pre)
                ...
                if (checkpoint_iterations and i % checkpoint_iterations == 0) or last_step:
                    ...
                    yield (
                        ...
                        vgg.unprocess(best[0,:,:,:].reshape(shape[1:]), mean_pixel)  # Because we now have batch, choose the first one in the batch.
                    )
...
def input_pyramid(name, M, batch_size, k=5, with_content_image = False):
    ...
    """
    with tf.get_default_graph().name_scope(name):
        return_val = [tf.placeholder(tf.float32, [batch_size, M//(2**x), M//(2**x), 6 if with_content_image else 3], name=str(x)) for x in range(k)]
        return_val.reverse()
    return return_val
...
def noise_pyramid_w_content_img(M, batch_size, content_image, k=5):
    ...
    return [np.concatenate((np.random.rand(batch_size, M//(2**x), M//(2**x), 3),
                            np.array([scipy.misc.imresize(content_image[0], (M//(2**x), M//(2**x), 3))])), axis = 3) for x in range(k)][::-1]
