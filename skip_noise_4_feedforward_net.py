...
def net(image, mirror_padding=True, reuse=False):
    ...
    with tf.variable_scope('skip_noise_4', reuse=reuse):
        for i in range(len(nums_3x3down)):
            ...
            skip_conv_prev = _conv_layer(prev_layer, nums_1x1[i], 1, 1, relu=False, mirror_padding=mirror_padding, name='skip_conv_prev_%d' %i, reuse=reuse)
            skip_conv_prev_shape = map(lambda i: i.value, skip_conv_prev.get_shape())
            ...
            skip_deeper_concat = tf.concat(3,[skip_concat,deeper_2], name='skip_deeper_concat_%d' %i)
            prev_layer = skip_deeper_concat

        # Next is deconv.
        for i in range(len(nums_3x3up)):
            deconv_1 = _conv_layer(prev_layer, nums_3x3up[i], 3, 1, mirror_padding=mirror_padding, name='deconv_1_%d' %i, reuse=reuse)
            deconv_2 = _conv_layer(deconv_1, nums_3x3up[i], 1, 1, mirror_padding=mirror_padding, name='deconv_2_%d' %i, reuse=reuse)
            prev_layer = deconv_2

        # Do a final convolution.
        weights_init = _conv_init_vars(prev_layer, 1, 1, name='final_conv', reuse=reuse)
