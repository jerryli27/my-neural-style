...
def style_synthesis_net(path_to_network, height, width, styles, iterations, batch_size,
    ...
    with tf.Graph().as_default():
        ...
        if not do_restore_and_generate:
            ...
            content_features_size = content_features_shape[0] * content_features_shape[1] * content_features_shape[2] * \
                                    content_features_shape[3]
            content_loss = content_weight * (2 * tf.nn.l2_loss(
                net[CONTENT_LAYER] - content_features) / content_features_size)

            if use_semantic_masks:
                content_semantic_mask = tf.placeholder('float', shape=[batch_size, input_shape[1], input_shape[2], semantic_masks_num_layers], name='content_mask')
            ...
            for layer in STYLE_LAYERS:
                ...
                if use_mrf:
                    if use_semantic_masks:
                        current_feature = neural_doodle_util.concatenate_mask_layer_tf(output_semantic_mask_features[style_layer], current_feature)
                    style_features[layer] = current_feature
                else:
                    if use_semantic_masks:
                        current_feature = neural_doodle_util.vgg_layer_dot_mask(output_semantic_mask_features[style_layer], current_feature)
            ...
            # TODO: tell which one is better, training all variables or training only scale and offset.
                ...
                if multiple_styles_train_scale_offset_only:
                    ...
                    train_step = tf.train.AdamOptimizer(learning_rate_decayed, beta1=0.9, beta2=0.999).minimize(
                        overall_loss)
        ...
        with tf.Session() as sess:
            if do_restore_and_generate:
                ...
                if use_semantic_masks:
                    mask_dirs = get_all_image_paths_in_dir(style_folder)
                    # Ignore the ones at the end to avoid
                    if batch_size != 1 and len(mask_dirs) > batch_size:
                        mask_dirs = mask_dirs[:-(len(mask_dirs) % batch_size)]
                    assert len(mask_dirs) == len(style_dirs)
                ...
                for i in range(iter_start, iterations):
                    ...
                    if use_semantic_masks:
                        # Load masks images
                        current_mask_dirs = get_batch(mask_dirs, i * batch_size, batch_size)
                        mask_pre_list = read_and_resize_batch_images(current_mask_dirs, input_shape[1], input_shape[2])
