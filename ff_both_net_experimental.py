...
from neural_util import gramian, total_variation
...
def style_synthesis_net(path_to_network, height, width, styles, iterations, batch_size,
    ...
    with tf.Graph().as_default():
        ...
        style_images = tf.placeholder(tf.float32, [1, input_shape[1], input_shape[2], 3],
        ...
        for layer in STYLE_LAYERS:
            if use_mrf or use_semantic_masks:
                style_features[0][layer] = style_net[layer]
            else:
                # Calculate and store gramian.
                style_features[0][layer] = gramian(style_net[layer])
            ...
            if do_restore_and_generate:
                ...
                if use_johnson:
                    ...
                    if content_img_style_weight_mask is not None:
                        ...
                        image = johnson_feedforward_net_util.net(inputs_concatenated_with_style, one_hot_style_vector=one_hot_style_vector, reuse=True)
                elif use_skip_noise_4:
                    if use_semantic_masks:
                        inputs = tf.placeholder(tf.float32, shape=[batch_size, input_shape[1], input_shape[2], semantic_masks_num_layers + NUM_NOISE_LAYERS * semantic_masks_num_layers + NUM_NOISE_LAYERS])
                    else:
                ...
                while from_screenshot or from_webcam or (iterator == 0):
                    ...
                    feed_dict[style_images] = np.array([styles[0]])
                ...
                for i in range(iter_start, iterations):
                    ...
                    for style_i in range(len(styles)):
                        ...
                        feed_dict[style_images] = style_pre_list
                            ...
                            if style_i == len(styles) - 1:
                                ...
                                if test_img_dir is not None:
                                    if use_semantic_masks:
                                        ...
                                        test_image_shape = test_image.shape
                                    styles[0] = scipy.misc.imresize(styles[0], (test_image_shape[0], test_image_shape[1]))
